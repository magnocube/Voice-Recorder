...
static void  captdnsRecv(struct sockaddr_in *premote_addr, char *pusrdata, unsigned short length) {
	...
	for (i=0; i<my_ntohs(&hdr->qdcount); i++) 
	{
		...
		printf("DNS: Q (type 0x%X classs 0x%X) for %s\n", my_ntohs(&qf->type), my_ntohs(&qf->classs), buff);
...
void captdnsInit(void) 
{
      ...
      xTaskCreatePinnedToCore((TaskFunction_t)captdnsTask,		            //task function		   //hanles the big button on the case(when an interupt is send)
							 "dns_task", 					                //task name 
							 1024 * 8, 											//stack size
							 NULL,												//function parameters (struct with pointers to shared classses)
							 1,													//priority
							 NULL,												//task handle
							 1													//task core
							 );
