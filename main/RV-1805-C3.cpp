...
//instance of rtc class (too have it globally accessable)
RV1805 rtcInstance;  
...
bool RV1805::begin() // will use the pre-configured i2s interface (which will be made in a class)
{
...
uint8_t RV1805::readRegister(uint8_t addr)
{
    ...
    i2c_master_write_byte(cmd, (RV1805_ADDR << 1) | I2C_MASTER_WRITE, true /* expect ack */);
    i2c_master_write_byte(cmd, addr, 1 /* expect ack */);
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd,(RV1805_ADDR << 1) | I2C_MASTER_READ, true /* expect ack */); 
    ...
    if(espRc != ESP_OK){
        printf("error in I2C writing the command to rtc (while reading), %d \n" , espRc);
        return data;
    } else{ 
        //printf("succesfull reading, adress: %d ,Data received %d\n" , addr, data);
        return data;
    }
}
...
bool RV1805::writeRegister(uint8_t addr, uint8_t val)
{
    ...
    if(espRc != ESP_OK){
        printf("error in I2C writing the command to rtc\n");
        return false;
    } 
    return true;
}
...
bool RV1805::writeMultipleRegisters(uint8_t addr, uint8_t * values, uint8_t len)
{
    ...
    if(espRc != ESP_OK){
        printf("error in I2C writing the command(s) to rtc\n");
        return false;
    } 
    return true;
}
...
bool RV1805::readMultipleRegisters(uint8_t addr, uint8_t * dest, uint8_t len)
{
	...
	{
		//dest[i] = _i2cPort->read();
        i2c_master_read_byte(cmd,&dest[i],I2C_MASTER_ACK);
	}
